            ...
            <div class="card">
                ...
                <DataTable :value="customer1" :paginator="true" :rows="15" dataKey="id" :rowHover="true" v-model:filters="filters1" filterDisplay="menu" :loading="loading1" :filters="filters1" :globalFilterFields="['name', 'status']">
                    ...
                    <Column field="name" header="Related" style="min-width: 12rem">
                        <template #body="{ data }"> Related to: project link </template>
                        <template #filter="{ filterModel }">
                            <InputText type="text" v-model="filterModel.value" class="p-column-filter" placeholder="Search by name" />
                        </template>
                    </Column>
                    ...
                    <Column field="name" header="Details" style="width: 26rem">
                        <template #body>
                            <div class="flex align-items-center gap-2">
                                <span>Lorem ipsum dolor, sit amet consectetur adipisicing elit. Aperiam veniam soluta maiores consequuntur? Alias itaque</span>
                            </div>
                        </template>
                        <template #filter="{ filterModel }">
                            <InputText type="text" v-model="filterModel.value" class="p-column-filter" placeholder="Search by name" />
                ...
                <!-- Create dialog -->
                <Dialog header="Add Invoice" v-model:visible="uploadDialog" :modal="true" style="max-height: 500px" class="add-product-dialog">
